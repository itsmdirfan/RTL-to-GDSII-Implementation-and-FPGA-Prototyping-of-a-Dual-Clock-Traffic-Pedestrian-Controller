...
module top(clk_main, clk_ped, rst_main_n, rst_ped_n, button,
  ...
  pc3d01 m_rst(.PAD(rst_main_n),.CIN(rst1));
  pc3c01 m_clk(.CCLK(clk_main),.CP(clk1));
  
  pc3d01 p_rst(.PAD(rst_ped_n),.CIN(rst2));
  pc3c01 p_clk(.CCLK(clk_ped),.CP(clk2));
  
  pc3d01 t_light0(.PAD(traffic_light[0]),.CIN(t0));
  pc3d01 t_light1(.PAD(traffic_light[1]),.CIN(t1));
  pc3d01 p_walk(.PAD(walk),.CIN(walk1)); 
  pc3d01 p_button(.PAD(button),.CIN(button1));    
